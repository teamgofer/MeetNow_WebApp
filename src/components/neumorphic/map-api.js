...
window.mapAPI = (function() {
        ...
        if (data.results && data.results.length > 0) {
          return data.results.map(result => {
            ...
            return {
              ...
              coordinates: result.coordinates || { latitude: lat + randomOffset() * 0.01, longitude: lon + randomOffset() * 0.01 },
              aqi: aqi,
    ...
    calculateAQI(pm25) {
      ...
      for (const bp of breakpoints) {
        if (pm25 >= bp.min && pm25 <= bp.max) {
          ...
          return Math.round(((bp.maxAQI - bp.minAQI) / (bp.max - bp.min)) * (pm25 - bp.min) + bp.minAQI);
        }
      }
  ...
  const sunshine = {
    ...
    calculateSunAreas(sunPosition, sunrise, sunset) {
      const now = new Date();
      const isMorning = now >= sunrise && now < new Date(sunrise.getTime() + (sunset - sunrise) * 0.4);
      const isAfternoon = now >= new Date(sunrise.getTime() + (sunset - sunrise) * 0.6) && now < sunset;
  ...
  const weather = {
    ...
    getWeatherIconClass(weatherType) {
      switch (weatherType) {
        case 'rain': return 'neu-weather-icon-rain';
        case 'snow': return 'neu-weather-icon-snow';
        case 'cloudy': return 'neu-weather-icon-cloudy';
        case 'partly-cloudy': return 'neu-weather-icon-partly-cloudy';
        default: return 'neu-weather-icon-clear';
      }
    }
  };
