...
const ProximityMessageList = ({
  ...
  const { messages, userLocation, currentUserId, chatSettings } = useProximityChatContext();
  ...
  useEffect(() => {
    ...
    const filteredMessages = filterMessagesByProximity(messages, userLocation, {
      proximityRadius,
      currentUserId,
      isUserBlocked,
    });
    ...
    const enhancedMessages = enhanceMessagesWithProximityData(filteredMessages, userLocation);

    // Track which messages are entering/leaving proximity
    const newMessages = getNewProximityMessages(enhancedMessages, prevMessagesRef.current);
    ...
    return (
      <div className="proximity-message-list location-required">
        <div className="location-required-message">
          ...
          <p>Please enable location to see messages from nearby users.</p>
        </div>
      </div>
    );
  }
