...
const SearchFilter = ({ className = '' }) => {
  ...
  return (
    <div className={`search-filter ${className}`} data-testid="search-filter">
      ...
      <FeatureFlag flag="USE_EXTRACTED_DISTANCE_SLIDER" fallback={<OriginalDistanceSlider />}>
        <DistanceSlider
      ...
      <FeatureFlag flag="USE_EXTRACTED_CATEGORY_FILTER" fallback={<OriginalCategoryFilter />}>
        <CategoryFilter
      ...
      <FeatureFlag flag="USE_EXTRACTED_TIME_FILTER" fallback={<OriginalTimeFilter />}>
        <TimeFilter
      ...
      {process.env.NODE_ENV === 'development' && (
        <div
          className="development-indicators"
          style={{ fontSize: '10px', marginTop: '16px', opacity: 0.6 }}
        >
          <div>
            DistanceSlider:{' '}
            {isFeatureEnabled('USE_EXTRACTED_DISTANCE_SLIDER') ? '✅ Extracted' : '⚙️ Original'}
          </div>
          <div>
            CategoryFilter:{' '}
            {isFeatureEnabled('USE_EXTRACTED_CATEGORY_FILTER') ? '✅ Extracted' : '⚙️ Original'}
          </div>
          <div>
            TimeFilter:{' '}
            {isFeatureEnabled('USE_EXTRACTED_TIME_FILTER') ? '✅ Extracted' : '⚙️ Original'}
          </div>
        </div>
      )}
    </div>
