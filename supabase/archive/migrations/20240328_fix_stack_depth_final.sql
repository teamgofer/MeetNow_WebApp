-- Final fix for stack depth and function overload issues
...
DECLARE
  func_name text;
BEGIN
  -- Drop all versions of create_meetup
  FOR func_name IN (
    SELECT format('%s(%s)', p.proname, pg_get_function_arguments(p.oid))
    FROM pg_proc p
  ...
  -- Drop all versions of create_free_meetup
  FOR func_name IN (
    SELECT format('%s(%s)', p.proname, pg_get_function_arguments(p.oid))
    FROM pg_proc p
    JOIN pg_namespace n ON p.pronamespace = n.oid
