 ...
 * @param location - Location object with lat and lng properties
 * @returns PostGIS point string (POINT(lng lat))
 */
export const toPostGISPoint = (location: { lat: number; lng: number }): string => {
 ...
 * @param postgisPoint - PostGIS point object from the database
 * @returns Location object with lat and lng properties
 */
export function fromPostGISPoint(postgisPoint: any): { lat: number; lng: number } {
  try {
    ...
    if (typeof postgisPoint === 'object' && postgisPoint !== null) {
      ...
      if (typeof postgisPoint.lat === 'number' && typeof postgisPoint.lng === 'number') {
        console.log(
          `[DEBUG] Using direct lat/lng object: {lat: ${postgisPoint.lat}, lng: ${postgisPoint.lng}}`
        );
    ...
    if (typeof postgisPoint === 'string' && postgisPoint.startsWith('01')) {
      ...
      const KNOWN_POSTGIS_POINTS: { [key: string]: { lat: number; lng: number } } = {
      ...
      try {
        ...
        if (isNaN(lat) || isNaN(lng) || lat < -90 || lat > 90 || lng < -180 || lng > 180) {
          console.error(
            `[DEBUG] Parsed coordinates out of valid range: {lat: ${lat}, lng: ${lng}}`
          );
          return getFallbackLocation();
        }
...
function reverseEndianness(hexString: string): string {
...
function hexToDouble(hexString: string): number {
 ...
 * @param meetupData - Meetup data from database
 * @returns Processed meetup data with location in lat/lng format
 */
export const processPostGISMeetups = (meetupData: any): any => {
...
/**
 * Get search radius in meters based on map zoom level
 * @param zoomLevel - Map zoom level (typically 1-20)
 * @returns Search radius in meters
 */
export const getSearchRadiusFromZoom = (zoomLevel: number): number => {
  if (zoomLevel >= 18) return 250; // 250m
  if (zoomLevel >= 16) return 1000; // 1km
  if (zoomLevel >= 14) return 2500; // 2.5km
  if (zoomLevel >= 12) return 5000; // 5km
  if (zoomLevel >= 10) return 10000; // 10km
  if (zoomLevel >= 8) return 25000; // 25km
  return 50000; // 50km
};
