...
ADD COLUMN IF NOT EXISTS starts_at TIMESTAMPTZ DEFAULT CURRENT_TIMESTAMP;

-- Add current_participants column if it doesn't exist
ALTER TABLE public.meetups 
ADD COLUMN IF NOT EXISTS current_participants INTEGER DEFAULT 1;

-- Add is_free_meetup column if it doesn't exist
ALTER TABLE public.meetups 
ADD COLUMN IF NOT EXISTS is_free_meetup BOOLEAN DEFAULT false;

-- Update the create_free_meetup function to use the new schema
