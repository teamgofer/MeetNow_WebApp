...
describe('MapNavigationController', () => {
  ...
  test('navigateTo should reject invalid locations', async () => {
    ...
    // Wrong type for lat
    await expect(navigator.navigateTo({ lat: 'test', lng: -118.243 })).rejects.toThrow(
      'Invalid location'
    );
  ...
  test('convenience methods should call navigateTo with correct parameters', async () => {
    ...
    expect(navigateToSpy).toHaveBeenCalledWith(location, expect.objectContaining({ mode: 'pan' }));
