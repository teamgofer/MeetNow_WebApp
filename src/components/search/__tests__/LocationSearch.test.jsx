...
describe('LocationSearch Component', () => {
  ...
  test('renders with default props', () => {
    ...
    expect(screen.getByTestId('location-search-input')).toHaveAttribute(
      'placeholder',
  ...
  test('renders with custom placeholder', () => {
    render(<LocationSearch onLocationSelect={() => {}} placeholder="Enter an address" />);
  ...
  test('shows initial query when provided', () => {
    render(<LocationSearch onLocationSelect={() => {}} initialQuery="New York" />);
  ...
  test('triggers search when query is at least 3 characters', async () => {
    ...
    await waitFor(() => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('/api/geocode?query=San'),
        expect.anything()
      );
    });
  });
