  ...
  const handleImageChange = async (e) => {
    ...
    try {
      ...
      if (file.size > 5 * 1024 * 1024) {
        throw createValidationError('Image size must be less than 5MB', { fileSize: file.size });
      }

      // Validate file type
      if (!file.type.startsWith('image/')) {
        throw createValidationError('File must be an image', { fileType: file.type });
      }
      ...
      // Upload image
      const { success, imageUrl, signedViewUrl, error } = await uploadImage(file, isAnonymous);
      
      if (!success || error) {
        throw createStorageError(error?.message || 'Failed to upload image', { 
          fileSize: file.size,
          fileType: file.type
        });
      }
      ...
      setSignedViewUrl(signedViewUrl);
      onImageUploaded(imageUrl, signedViewUrl);
    } catch (error) {
      const result = await handleError(error, error.type || ErrorTypes.STORAGE, {
        component: 'MeetupImageUploader',
