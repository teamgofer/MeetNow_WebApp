...
class NavigationCore {
  ...
  updateState(newState) {
    ...
    if (newState.selectedLocation !== undefined) {
      if (newState.selectedLocation === null || isValidLocation(newState.selectedLocation)) {
        if (
          JSON.stringify(newState.selectedLocation) !== JSON.stringify(this.state.selectedLocation)
        ) {
  ...
  _changeMode(newMode) {
    ...
    if (!currentModeDefinition.allowedTransitions.includes(newMode)) {
      this.logValidationError(`Transition from ${this.state.mode} to ${newMode} not allowed`);
      return false;
    }
  ...
  _enterBirdsEyeMode(state) {
    ...
    if (!state.userLocation || !state.selectedLocation) {
      this.logValidationError("Bird's Eye View mode requires both user and selected locations", {
  ...
  _enterVicinityMode(state) {
    ...
    if (!state.userLocation) {
      this.logValidationError('Vicinity mode requires user location', {
        userLocation: state.userLocation,
      });
      return false;
    }
