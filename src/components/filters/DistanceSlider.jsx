...
const DistanceSlider = ({ className = '', initialValue = 5, min = 1, max = 50, onChange }) => {
  ...
  const [sliderValue, setSliderValue] = useState(meetupState.filters.distance || initialValue);
  ...
  return (
    <div
      className={`distance-slider ${className}`}
...
const EnhancedDistanceSlider =
  isFeatureEnabled('ENABLE_COMPONENT_REGISTRY') || process.env.NODE_ENV === 'development'
