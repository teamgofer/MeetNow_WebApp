...
export function ProximityChatProvider({ children }) {
  ...
  const sendMessage = useCallback(
    ...
    [
      ...
      state.userLocation,
      state.sessionId,
      state.chatSettings.anonymousMode,
      webSocketService,
    ]
  );
  ...
  const markMessagesAsRead = useCallback(
    (messageIds = null) => {
      ...
      const idsToMark =
        messageIds || getUnreadMessages(state.messages, state.sessionId).map(message => message.id);
      ...
      idsToMark.forEach(id => {
        const receiptUpdate = createReceiptUpdateMessage(id, ReceiptStatus.READ, state.sessionId);
  ...
  const trackRegionEntry = useCallback(
    regionId => {
  ...
  const getRegionEntryTime = useCallback(
    regionId => {
  ...
  const loadMessagesBeforeArrival = useCallback(
    async regionId => {
      const entryTime = getRegionEntryTime(regionId);
      if (!entryTime) return [];
      ...
      try {
        ...
        const messages = await messageHistoryService.loadMessagesBeforeArrival(regionId, entryTime);
  ...
  const loadOlderMessages = useCallback(
    async (regionId, oldestTimestamp) => {
      if (
  ...
  const resetHistoryStatus = useCallback(
    regionId => {
  ...
  return (
    <ErrorBoundary onError={handleProximityChatError} fallback={<>{children}</>}>
      <ProximityChatContext.Provider value={contextValue}>{children}</ProximityChatContext.Provider>
    </ErrorBoundary>
  );
}
