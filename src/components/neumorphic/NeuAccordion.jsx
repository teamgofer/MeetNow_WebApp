...
export const NeuAccordionItem = ({
  ...
  return (
    <div
      className={`neu-accordion-item ${getVariantClass()} ${isOpen ? 'neu-accordion-item-open' : ''} ${className}`}
    >
      <button className="neu-accordion-header" onClick={handleToggle} aria-expanded={isOpen}>
      ...
      <div
        ...
        ref={contentRef}
      >
        <div className="neu-accordion-content-inner">{children}</div>
      </div>
    </div>
  );
};
...
export const NeuAccordion = ({
  ...
  return (
    <div className={`neu-accordion ${getVariantClass()} ${className}`}>{enhancedChildren}</div>
  );
};
