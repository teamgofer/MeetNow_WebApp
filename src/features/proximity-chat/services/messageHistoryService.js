...
class MessageHistoryService {
  ...
  _trackMemoryUsage(messages) {
    ...
    this._monitor.trackMemoryUsage('messageHistory', 'messageSet', messageSize, {
      ...
      newestMessageTimestamp:
        messages.length > 0 ? Math.max(...messages.map(m => m.timestamp)) : null,
    });
  ...
  _cleanupCache() {
    ...
    try {
      ...
      while (this._memoryUsage > this._maxCacheSize * 0.8) {
        // Clear until 80% of max
  ...
  async loadMessageHistory(regionId, options = {}) {
    ...
    try {
      ...
      if (cachedMessages) {
        this._monitor.trackCacheOperation(
          'messageHistory',
          'loadFromCache',
          true,
          JSON.stringify(cachedMessages).length,
          {
  ...
  async getVisitedRegions() {
    ...
    try {
      ...
      // Update internal state
      this._visitedRegions = new Map((data.regions || []).map(region => [region.id, region]));
...
export const markMessagesAsRead = async (regionId, options = {}) => {
  ...
  try {
    ...
    const response = await authFetch(`${API.BASE_URL}/regions/${regionId}/messages/read`, {
      method: 'POST',
      headers: {
