...
const ChatMessageList = ({ className = '' }) => {
  const { messages, isLoading, currentUserId, markMessagesAsRead } = useProximityChatContext();
  ...
  if (messages.length === 0 && !isLoading) {
    ...
    return (
      <div className={`chat-empty-state ${className}`}>
        ...
        <p className="empty-message">Be the first to send a message to nearby users!</p>
      </div>
    );
  }
      ...
      <div className="chat-message-list">
        {sortedMessages.map((message, index) => {
          ...
          return (
            ...
            >
              <ChatMessageItem message={message} previousMessage={previousMessage} />
            </div>
          );
        })}
