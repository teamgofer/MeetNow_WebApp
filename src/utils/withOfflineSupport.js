...
export const withOfflineSupport = (WrappedComponent, options = {}) => {
  ...
  return function WithOfflineSupport(props) {
    ...
    // Save data to cache
    const saveToCache = React.useCallback((newData) => {
      if (cacheKey && shouldCache) {
        try {
    ...
    const handleRequest = React.useCallback(async (requestFn, ...args) => {
      setIsLoading(true);
      setError(null);
    ...
    // Force refresh data
    const refresh = React.useCallback(async (requestFn, ...args) => {
      clearCache();
      return handleRequest(requestFn, ...args);
