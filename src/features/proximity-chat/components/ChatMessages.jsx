...
const ChatMessages = ({ messages, currentUserId }) => {
  ...
  const groupedMessages = messages.reduce((groups, message) => {
    ...
    if (
      ...
      Math.abs(
        new Date(lastGroup.messages[lastGroup.messages.length - 1].timestamp) -
    ...
    <div className="flex-1 overflow-y-auto p-4 space-y-4">
      {groupedMessages.map((group, groupIndex) => {
        ...
        return (
          ...
          >
            <div
              className={`max-w-3/4 ${isCurrentUser ? 'bg-blue-500 text-white' : 'bg-gray-100 dark:bg-gray-700 dark:text-white'} rounded-lg p-1`}
            >
              {!isCurrentUser && (
              ...
              <div className="space-y-1 p-2">
                {group.messages.map(message => (
                  <div key={message.id} className="break-words">
                    <p>{message.content}</p>
                    <div
                      className={`text-xs mt-1 ${isCurrentUser ? 'text-blue-200' : 'text-gray-500 dark:text-gray-400'}`}
                    >
