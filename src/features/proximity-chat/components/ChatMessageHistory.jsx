 ...
 */
const ChatMessageHistory = ({ regionName, regionTimeInfo, className = '', style = {} }) => {
  ...
  const { historyMessages, loading, error, hasMoreMessages, loadBeforeArrival, loadOlderMessages } =
    useMessageHistory({
  ...
  if (loading && historyMessages.length === 0) {
    return (
      <div className={`${styles.chatMessageHistory} ${className}`} style={style}>
