...
export const usePermissionCheck = (user, _permission, resource) => {
    if (!user?.id) {
        return false;
    }
...
export const sanitizeRequestParams = (req, _res, next) => {
    try {
        if (req.query) {
            Object.keys(req.query).forEach(key => {
                const value = req.query[key];
                if (typeof value === 'string') {
                    const suspicious = /(['";]|--|\b(select|insert|update|delete|drop|alter|create|exec)\b)/i.test(value);
                    if (suspicious) {
