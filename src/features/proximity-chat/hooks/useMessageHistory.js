 ...
 */
const useMessageHistory = ({ autoLoad = true, markAsReadOnUnmount = true, limit = 50 } = {}) => {
      ...
      try {
        ...
        if (cachedMessages) {
          ...
          setMessages(prevMessages =>
            beforeTimestamp ? [...prevMessages, ...cachedMessages] : cachedMessages
          );
          return;
        }
        ...
        const response = await fetch(
          `/api/messages?region=${region}&limit=${limit}${beforeTimestamp ? `&before=${beforeTimestamp}` : ''}`
        );
        ...
        // Update state
        setMessages(prevMessages =>
          beforeTimestamp ? [...prevMessages, ...newMessages] : newMessages
        );
