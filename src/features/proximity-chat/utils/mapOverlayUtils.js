...
export const calculateChatBubbleSize = (chatActivity, options = {}) => {
  const { messageCount = 0, uniqueUsers = 0, recentActivity = 0 } = chatActivity || {};

  const { minSize = 20, maxSize = DEFAULT_SETTINGS.chatBubbleMaxSize } = options;
      ...
      if (distance <= threshold) {
        ...
        cluster.center.latitude =
          (cluster.center.latitude * (cluster.totalWeight - weight) +
            otherLocation.latitude * weight) /
          cluster.totalWeight;

        cluster.center.longitude =
          (cluster.center.longitude * (cluster.totalWeight - weight) +
            otherLocation.longitude * weight) /
          cluster.totalWeight;
      }
    });
...
export const identifyChatHotspots = (messages, users, options = {}) => {
  ...
  return locationClusters
    .map(cluster => {
      ...
      const recentMessages = cluster.members.filter(m => new Date(m.timestamp) >= timeThreshold);

      // Find any active users in this area
      const nearbyUsers = users.filter(
        user => user.location && calculateDistance(user.location, cluster.center) <= clusterRadius
      );
...
export const getUserMarkerConfig = (user, privacySettings = {}) => {
  ...
  if (locationPrecision === 'approximate') {
    ...
    markerConfig.location.latitude =
      Math.round(markerConfig.location.latitude / gridSize) * gridSize;
    markerConfig.location.longitude =
      Math.round(markerConfig.location.longitude / gridSize) * gridSize;
    markerConfig.approximateLocation = true;
  }
