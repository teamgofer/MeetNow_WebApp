...
export async function getNearbyFreeMeetups(
  location: { lat: number; lng: number },
  radius: number = 5000
) {
  try {
    ...
    console.log(
      `Found ${filteredMeetups.length} nearby meetups after filtering by proximity to USER`
    );
...
export async function nearbyMeetups(
  location: { lat: number; lng: number },
  radius: number = 5000
): Promise<any[]> {
  try {
