...
const AccessibilityInlineHelp = ({
  ...
  return (
    <div className={`a11y-help-container a11y-help-${placement}`}>
      ...
      <button className="a11y-help-toggle" {...buttonProps} aria-expanded={isExpanded}>
      ...
      {isExpanded && (
        <div className="a11y-help-content" role="tooltip" id={`help-${featureId}`}>
                ...
                {relatedActions.map(action => {
                  ...
                  return (
                    <li key={action.id} className="a11y-help-shortcut-item">
                      <span className="a11y-help-action-name">{action.label}</span>
                      {shortcutKey ? (
                        <span className="a11y-help-shortcut-key">
                          {formatShortcut(shortcutKey)}
                        </span>
                      ) : (
                        <span className="a11y-help-no-shortcut">No shortcut available</span>
                      )}
                    </li>
                  );
                })}
          ...
          <button className="a11y-help-close" onClick={toggleHelp} aria-label="Close help">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="16"
              height="16"
