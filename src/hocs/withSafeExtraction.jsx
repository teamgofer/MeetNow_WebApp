...
const withSafeExtraction = (
  WrappedComponent,
  { id = null, dependencies = [], errorBoundary = true, defaultProps = {}, onExtract = null } = {}
) => {
  // Create component ID if not provided
  const componentId =
    id ||
    `${WrappedComponent.displayName || WrappedComponent.name || 'Component'}_${Math.random().toString(36).substr(2, 9)}`;
  ...
  const SafeExtractedComponent = ({ forwardedRef, ...props }) => {
    ...
    if (hasError && errorBoundary) {
      return (
        <div className="component-error-boundary" data-component-id={componentId} ref={ref}>
  ...
  return React.forwardRef((props, ref) => <SafeExtractedComponent {...props} forwardedRef={ref} />);
};
