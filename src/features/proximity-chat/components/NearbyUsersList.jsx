...
const NearbyUsersList = ({ onUserSelect }) => {
  ...
  useEffect(() => {
    ...
    const entered = nearbyUsers.filter(
      user => !prevUserIds.has(user.id) && user.id !== currentUserId
    );

    // Find users who left the vicinity
    const left = prevUsers.filter(
      user => !currentUserIds.has(user.id) && user.id !== currentUserId
    );
  ...
  const renderUser = user => {
    ...
    const userElement = (
      <li key={user.id} className={`nearby-user-item ${isExpanded ? 'expanded' : ''}`}>
        <div className="nearby-user-main" onClick={() => toggleUserExpanded(user.id)}>
  ...
  return (
    <div className="nearby-users-container">
      ...
      {filteredUsers.length > 0 ? (
        <ul className="nearby-users-list">{filteredUsers.map(renderUser)}</ul>
      ) : (
      ...
      <div className="activity-section">
        ...
        <div className="activity-log" ref={activityLogRef}>
          {activities.length > 0 ? (
            <ul className="activity-list">{activities.map(renderActivity)}</ul>
          ) : (
