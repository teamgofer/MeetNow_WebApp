...
async function initRealPollutionOverlay(overlay, lat, lon) {
  ...
  try {
    ...
    pollutionData.forEach((station, index) => {
      ...
      const x =
        50 + (index - Math.floor(pollutionData.length / 2)) * 15 + (Math.random() - 0.5) * 10;
...
function updateFogOfWar(map, radius) {
  ...
  const userLat = parseFloat(
    map.getAttribute('data-user-lat') || map.getAttribute('data-lat') || '0'
  );
  const userLng = parseFloat(
    map.getAttribute('data-user-lng') || map.getAttribute('data-lon') || '0'
  );
