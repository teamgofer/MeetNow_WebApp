-- Test script for the dynamic property values with external data integration

-- Insert test properties if they don't exist
DO $$
DECLARE
  v_owner_id UUID;
BEGIN
...
-- Test the external population density function directly
SELECT 
  p.name,
  p.city,
  ST_Y(p.center_point::geometry) AS lat,
  ST_X(p.center_point::geometry) AS lng,
  get_external_population_density(
    ST_Y(p.center_point::geometry),
...
-- Test the points of interest function directly
SELECT 
  p.name,
  p.city,
  get_external_points_of_interest(
    ST_Y(p.center_point::geometry),
    ST_X(p.center_point::geometry),
    1000
  ) AS poi_data
...
-- Test the social trending function directly
SELECT 
  p.name,
  p.city,
  get_external_trending_data(
    ST_Y(p.center_point::geometry),
    ST_X(p.center_point::geometry)
  ) AS trending_data
FROM 
...
-- Now update property values using the dynamic calculation
SELECT p.id, p.name, calculate_dynamic_property_value(p.id) as result
...
-- Check what's in the API cache now
SELECT 
  cache_key, 
  source, 
  expires_at,
