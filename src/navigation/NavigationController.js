...
class NavigationController {
        ...
        this.listeners.selectedLocation.forEach(listener => {
          try {
            ...
            Logger.error(
              ...
              'Error in selected location change listener',
              error
            );
          }
        });
  ...
  showBirdsEyeView(locationA, locationB, options = {}) {
    ...
    return this._queueOperation(async () => {
      ...
      const bounds = L.latLngBounds([locationA.lat, locationA.lng], [locationB.lat, locationB.lng]);

      // Handle case where points are identical or very close
      const isIdentical =
        Math.abs(locationA.lat - locationB.lat) < 0.000001 &&
