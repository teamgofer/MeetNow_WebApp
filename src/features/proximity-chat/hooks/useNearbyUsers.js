...
const useNearbyUsers = customRadiusKm => {
  const { nearbyUsers, currentUserId, userLocation, chatSettings } = useProximityChatContext();
  ...
  const users = useMemo(() => {
    ...
    try {
      ...
      const filteredUsers = Object.values(nearbyUsers)
        ...
        // Filter by distance
        .filter(user => isLocationWithinRadius(user.location, userLocation, proximityRadius))
