  ...
  try {
    ...
    if (type === 'recent') {
      ...
      prefs.recentSearches = [
        ...
        ...prefs.recentSearches.filter(item => item.place_id !== location.place_id),
      ].slice(0, 5);
    } else {
      // Add to saved locations (max 10)
      // First remove if already exists to avoid duplicates
      prefs.locations = prefs.locations.filter(item => item.place_id !== location.place_id);

      // Then add to the beginning of the array
      prefs.locations = [locationWithTimestamp, ...prefs.locations].slice(0, 10);
    }
...
async function handleDeletePreference(req, res, userId) {
  try {
    ...
    if (type === 'recent') {
      prefs.recentSearches = prefs.recentSearches.filter(
